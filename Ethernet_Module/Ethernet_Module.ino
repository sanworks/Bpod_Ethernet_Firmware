...
const uint32_t MessageBufSize = 256;
...
byte MessageLibrary[N_MESSAGES][MAX_MESSAGE_LENGTH] = {0}; // Stores messages to send to server on trigger
byte MessageLibrarynBytes[N_MESSAGES] = {0}; // Stores the length of each message in bytes
uint32_t nBytesTransferred = 0;
byte nBytes2Relay = 0;
byte newByte = 0;
byte messageNum = 0;
byte messagenBytes = 0;
...
void loop() {
   ...
   if (client) {
      if (client.available() > 0) {
        ...
        opByte = client.read();
        switch (opByte) {
          case 'H': // Handshake (returns a 1)
            client.write(1);
          break;
          case 'R': // Relay 1 byte to state machine
            newByte = client.read();
            Serial1COM.writeByte(newByte);
            nBytesTransferred += 1; 
          break;
        }
      }
   ...
   if (Serial1COM.available() > 0) {
    ...
    switch (opByte) {
      case 255:
        returnModuleInfo();
      break;
      case 'T': // Trigger message transmission from library to current client (or server)
        messageNum = Serial1COM.readByte();
        messagenBytes = MessageLibrarynBytes[messageNum];
        for (int i = 0; i < messagenBytes; i++) {
          MessageBuffer[i] = MessageLibrary[messageNum][i];
        }
        client.write(MessageBuffer, messagenBytes);   
        nBytesTransferred += messagenBytes;
      break;
    }
   }
   if (USBCOM.available() > 0) {
    ...
    switch (opByte) {
      ...
      break;
      case 'R': // Relay message to current client (or server)
        
      break;
      case 'M': // Store message in library to be triggered in the future
        messageNum = USBCOM.readByte();
        messagenBytes = USBCOM.readByte();
        MessageLibrarynBytes[messageNum] = messagenBytes;
        USBCOM.readByteArray(MessageBuffer, messagenBytes);
        for (int i = 0; i < messagenBytes; i++) {
          MessageLibrary[messageNum][i] = MessageBuffer[i];
        }        
        USBCOM.writeByte(1);
      break;
      case 'T': // Trigger message transmission from library to current client (or server)
...
void returnModuleInfo() {
  ...
  Serial1COM.writeUint32(FIRMWARE_VER); // 4-byte firmware version
